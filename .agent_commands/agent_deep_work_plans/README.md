...
1. **Creation** - Plan folder and task files are generated
2. **Execution** - Agent works through tasks sequentially
3. **Completion** - All tasks marked as done
4. **Archive/Delete** - Plan folder can be removed (it's git-ignored)
5. **Documentation** - Important learnings are captured in permanent docs
...
**Goal:** Optimize Lambda performance (cold start and execution time)

**Status:** ✅ Complete reference example

**Location:** `example_prompts/PLAN_EXAMPLE_lambda_performance_optimization/`

**Based on:** DRAFT_EXAMPLE_refined.md (showing full workflow: draft → refined → plan)

**Structure:**

```text
example_prompts/PLAN_EXAMPLE_lambda_performance_optimization/
├─ README.md                                # Complete plan with 10 tasks
├─ PROMPTS.md                               # Ready-to-use prompts + performance tracking
├─ 1.task_establish_baseline.md            # Task 1: Detailed baseline analysis
└─ ... (tasks 2-10 for complete optimization flow)
```

**Use this as a reference** to see what a complete plan looks like with:

- ✅ Complete README with phased tasks (Analysis → Optimization → Testing)
- ✅ Detailed task file with CloudWatch metrics, validation, acceptance criteria
- ✅ PROMPTS.md with performance-specific prompts (progress reports, rollback)
- ✅ Realistic example for this Node.js/TypeScript/Lambda project
- ✅ Performance targets and success metrics clearly defined
- ✅ Proper TypeScript standards and validation commands

**Note:** This example shows the complete lifecycle from draft to plan:

1. Start with `DRAFT_EXAMPLE_basic.md` (quick idea)
2. Refine to `DRAFT_EXAMPLE_refined.md` (detailed prompt)
3. Create plan: `PLAN_EXAMPLE_lambda_performance_optimization/` (execution ready)
...
- **⭐ Practical examples:** `example_prompts/` - Copy-paste ready prompts (start here!)
  ...
  - [`PLAN_EXAMPLE_lambda_performance_optimization/`](./example_prompts/PLAN_EXAMPLE_lambda_performance_optimization/) - ⭐ Complete plan example
