...
- The original tier-3 design spec, if produced, lived in the third-level-tags taxonomy Deep Work Plan (historical legacy DWP plan output; not tracked).
...
- Per-post quota check: would adding this tag exceed its tier cap (1-3 primary, 1-10 secondary, 0-15 subtopic)? If yes, propose a tag to demote (lowest tier first; never primary, and never below the ≥1 secondary minimum).
