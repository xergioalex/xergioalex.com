...
> the examples below are illustrative, not prescriptive. Two further rules:
> a tier-wide verb such as `ncu -u --target patch` touches the whole tier —
> restrict actual mutations to the batch's approved set (`--filter`) per
> SPEC §4; and the **Revert** rows' `git checkout --` is valid **only when the
> pre-batch snapshot is proven identical to `HEAD`** — otherwise restore the
> recorded pre-batch snapshot, never a bare checkout over work the batch did
> not own (SPEC §6).
...
- **Revert from the recorded pre-batch snapshot**, not blindly from `HEAD` —
  `git checkout --` is correct only when proven identical to that snapshot
  (no earlier uncommitted batches, no pre-existing changes in the owned files).
