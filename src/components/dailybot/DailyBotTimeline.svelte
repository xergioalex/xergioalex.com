...
{#if posts.length === 0}
  <div class="text-center py-16">
    <p class="text-gray-600 dark:text-gray-300 text-lg">{t.dailybotPage.emptyState}</p>
  </div>
{:else}
          ...
          >
            ...
            <div class="p-5">
              <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-2 group-hover:text-secondary transition-colors">
                <a href={`${prefix}/blog/${slug}/`} class="hover:text-secondary transition-colors" on:click={() => trackEvent(EVENTS.TIMELINE_CLICK, { page: 'dailybot', slug })}>
                  {post.data.title}
                </a>
              </h3>
