...
{:else}
  ...
  {#if allLoaded && renderedSeries.length > 0}
    <div class="flex justify-center pt-8 pb-4">
      <div class="flex items-center gap-2 px-4 py-2 rounded-full bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700">
        <svg class="w-4 h-4 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
        </svg>
        <span class="text-sm text-gray-600 dark:text-gray-300">
          {lang === 'es' ? 'Todas las series cargadas' : 'All series loaded'}
        </span>
      </div>
    </div>
  {/if}
