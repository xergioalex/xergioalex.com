...
Almost all modern software is built by reusing code other people wrote. We call those reusable pieces *packages*, and they get downloaded automatically every time a developer (or, increasingly, an AI agent acting on their behalf) builds or ships an application. There are millions of them in public registries. And over the past year, several of the most popular ones have been hijacked to ship malicious code.

In May 2026, [42 packages in the `@tanstack/*` organization](https://tanstack.com/blog/npm-supply-chain-compromise-postmortem) were compromised inside a six-minute window; eight days later, [hundreds more under `@antv/*`](https://www.microsoft.com/en-us/security/blog/2026/05/20/mini-shai-hulud-compromised-antv-npm-packages-enable-ci-cd-credential-theft/) fell in 22 minutes. In April, [Bitwarden's command-line tool](https://www.paloaltonetworks.com/blog/cloud-security/bitwardencli-supply-chain-attack/) was under attacker control for ninety minutes — and the first thing the malicious code did once it reached a machine was scan for installed AI coding assistants. In March, [axios](https://socket.dev/blog/axios-npm-package-compromised) — a library used by thousands of companies, over 100 million weekly downloads — shipped two malicious versions over a 39-minute window. And in September 2025, npm saw [its first self-replicating worm](https://www.stepsecurity.io/blog/ctrl-tinycolor-and-40-npm-packages-compromised): code that, once installed on a developer's machine, harvested their credentials and republished itself into other packages owned by the same person — infecting hundreds within days.

If you ship software in 2026, the repositories you depend on are under coordinated attack. Malicious packages have always existed — the new thing is the speed, the scale, and AI showing up on both sides of the table. On the attacker side: AI is being used to invent plausible-sounding package names, to re-obfuscate code between victims, and to target the AI coding assistants themselves. On the victim side: those assistants are running `npm install` for us with less and less human review. A poisoned agent writes poisoned code for months.

This post is a tour of what's happening, who's getting hit, and how AI is reshaping the playbook. At the end, a concrete defensive baseline — the one I just shipped on this site — for developers who want to close the gap. Almost every citation links to a primary source: vendor postmortems, security-firm research, CISA advisories. None of this is speculation.
...
**Shai-Hulud (September 2025).** The first self-replicating worm in npm history — and the event that defined the rest of the year. A computer *worm* is code that copies itself from one victim to another; in this case, once a developer installed the compromised version of the `@ctrl/tinycolor` package, the code scanned their machine for credentials (Amazon Web Services keys, GitHub access, the npm publish token that lets the package owner ship new versions) and used that last token to infect up to twenty more packages owned by the same person. [CISA issued an alert](https://www.cisa.gov/news-events/alerts/2025/09/23/widespread-supply-chain-compromise-impacting-npm-ecosystem) on September 23. By the end of the month the infected-package count had grown from "180+" to "500+". [A second wave hit two months later](https://unit42.paloaltonetworks.com/npm-supply-chain-attack/), this time with a destructive fallback: if the code couldn't exfiltrate credentials cleanly, it wiped the user's home directory. [StepSecurity has the most detailed technical analysis](https://www.stepsecurity.io/blog/ctrl-tinycolor-and-40-npm-packages-compromised).
...
You can see the pattern across all of the above. The registry-level attack and the publishing-pipeline-level attack are converging. Developer credentials are stolen, their pipelines are leveraged, the malicious code runs both on personal machines and inside companies' internal systems, and the loot — ironically — is more credentials to keep expanding.
...
Two terms need defining first. **Typosquatting** is when an attacker registers a package with a name nearly identical to a legitimate one (`requets` instead of `requests`, `loadsh` instead of `lodash`) hoping someone installs it by mistyping. It's been around forever.
