...
Esta es una regla de internet que nadie te enseña: cada dominio tiene una puerta a la que cualquiera puede llamar sin pedir permiso. La has usado cientos de veces sin saber su nombre. Piensa en la última vez que entraste a una página que no conocías y le diste a «Iniciar sesión con Google». El botón simplemente funcionó. ¿De dónde sacó esa página la dirección de Google? Nadie reparte una lista oficial. Funciona porque Google — y Apple, y Microsoft — prometieron contestar siempre en la misma dirección, y todas las páginas del mundo saben que ahí se puede llamar.

Esa dirección es una carpeta: `/.well-known/`. No es una metáfora — es una ruta fija que la [RFC 8615](https://www.rfc-editor.org/rfc/rfc8615) (los RFC son los documentos con los que Internet define sus reglas) reservó hace años para exactamente eso: *lo que un sitio quiere que el mundo sepa de antemano, vive aquí*. Los clientes la llevan usando una década: `openid-configuration` — la dirección de la historia del botón —, `security.txt` para reportar vulnerabilidades, los desafíos de ACME que renuevan los certificados de tu sitio mientras duermes.
