...
Pero lo que más se me queda es la perspectiva más amplia. Durante 500 años, los tipógrafos de imprenta tuvieron control línea por línea sobre el layout del texto. Podían ajustar el ancho, el espaciado, la posición de cada línea. Cada línea era una decisión. En la web, el motor de layout del navegador abstrae casi todo eso — defines un ancho de contenedor y esperas lo mejor. El navegador decide los saltos de línea. Tú tienes muy poca voz en el comportamiento línea por línea.

PreTeXt devuelve ese control. No a la imprenta. Al navegador. A JavaScript. A nosotros.

Y se toma Unicode en serio — gracias a [`Intl.Segmenter`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/Segmenter), el texto CJK puede partir entre caracteres, árabe y hebreo corren de derecha a izquierda, tailandés usa partición de palabras basada en diccionario, y los clusters de emoji se mantienen juntos. Prueba el [test de estrés multilingüe](https://pretext.xergioalex.com/demos/i18n-stress/) — el demo cubre 9 muestras y casos límite de distintos sistemas de escritura. Muchas utilidades ligeras de texto en JavaScript empiezan a mostrar grietas apenas sales del texto latino simple. PreTeXt claramente está apuntando a ese problema más difícil. Cero dependencias externas — solo APIs del navegador que ya existen.
