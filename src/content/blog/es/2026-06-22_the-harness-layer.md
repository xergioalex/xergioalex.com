...
Mientras lo investigaba, un agente me entregó una estadística perfecta: que cerca del 70% del desempeño de un agente vive por fuera del modelo, en el harness. Limpia, citable, justo el tipo de cifra que ancla un ensayo como este. También era falsa — no se rastrea a ninguna fuente. Estuve a punto de publicarla; no lo hice porque el plan que seguía obligaba a verificar cada cifra contra su fuente primaria antes de escribirla, y esa no sobrevivió el paso. El agente puede entregarme una mentira con cara seria todas las veces que quiera. El error no llega al post publicado, porque algo entre el agente y `main` lo está revisando.

Ese movimiento pequeño — *el agente cometió un error, así que diseñé una solución para que el error dejara de existir* — resulta que ahora tiene nombre. La gente lo llama **harness engineering** (ingeniería del arnés que envuelve al agente), y en los últimos meses pasó de ser una frase que usó una sola persona en un blog a ser eso que muchos estamos descubriendo que veníamos haciendo desde siempre, sin una palabra para nombrarlo.
...
Eso es todo. Esa es la disciplina. No un framework, no un producto — un hábito. Cada error se vuelve un arreglo permanente, para que el mismo error no pueda volver. El paso de verificación que cazó esa estadística falsa es esa frase hecha literal.
...
Fowler traza además una línea a la que vuelvo seguido, entre dos tipos de revisión. Algunas son **computational** (computacionales) — deterministas y rápidas, del tipo que una máquina zanja con un sí o un no: un test pasa o falla, un tipo cuadra o no, un linter está contento o no lo está, un grep encuentra una tilde faltante o no la encuentra. Otras son **inferential** (inferenciales) — semánticas y más lentas, donde le pides a otro modelo que juzgue si un texto está en la voz correcta o si un cambio es de verdad seguro. Quieres que las revisiones computacionales hagan todo lo que puedan, porque son baratas y nunca se cansan; reservas las inferenciales para los juicios que una expresión regular no puede hacer.
...
Es el mismo movimiento, solo que en el extremo del dial. El paso que cazó esa estadística inventada y el millón de líneas escritas por agentes de OpenAI son el mismo instinto a distintas escalas: no te vuelvas mejor corrigiendo al agente, vuélvete mejor construyendo el sistema que hace que corregirlo sea innecesario.
