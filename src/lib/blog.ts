...
export async function getBlogPosts(params: BlogParamsType): Promise<BlogPostsResultType> {
  ...
  if (params.tag) {
    ...
    console.log(`Filtrado por tag "${params.tag}": ${posts.length} posts encontrados`);
  }
  ...
  if (params.page) {
    ...
    console.log(`Página ${params.page}: posts ${startIndex} a ${endIndex} (${posts.length} posts)`);
  } else {
  ...
  console.log(`Resultado: ${posts.length} posts en página ${result.currentPage} de ${totalPages} páginas`);
  return result;
}