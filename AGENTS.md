...
**Tags:** Flat `tags` array in frontmatter. Three tiers (primary / secondary / subtopic) resolved at build time from `src/content/tags/*.md`. Per-post tag caps: **1-3 primary, 0-10 secondary, 0-15 subtopic** (≥ 1 primary required; no fixed total cap — tag only what the post genuinely covers, never tag for the sake of tagging). Never auto-create tags without user approval — propose with [`/audit-taxonomy`](.agents/skills/audit-taxonomy/SKILL.md) and let the user approve. See [Tag Taxonomy in BLOG_POSTS.md](docs/features/BLOG_POSTS.md#tag-taxonomy-unified-collection) for the full tier table.
